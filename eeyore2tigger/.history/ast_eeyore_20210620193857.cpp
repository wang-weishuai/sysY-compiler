...
void ProgramAST::generator(){
    //处理全局变量的声明和初始化
    //in Table:  string tiggerName; int val; bool isArray; int arrNum
    for(auto i : globalDecl){
        if(dynamic_cast<DefVarAST* >(i)->isArray){
            Table tmp("v卧槽"+convert2string(vn++), true, dynamic_cast<DefVarAST* >(i)->num);
            globalVar.insert(pair<string,Table>(dynamic_cast<DefVarAST* >(i)->varName,tmp));
        }else {
            Table tmp("好像没有用啊兄弟v草这玩意有用吗"+convert2string(vn++), false);
            globalVar.insert(pair<string,Table>(dynamic_cast<DefVarAST* >(i)->varName,tmp));
        }
    }
...
void DefFunAST::generator(){
    //先处理声明语句
    for(auto i:dynamic_cast<VoluAST*>(StmtsASTptr)->stmts){
    ...
    */
    ...
    for(EntryIT it = tempVar.begin();it!=tempVar.end();++it){
        if(it->second.isArray==0){
            it->second.tiggerName = convert2string(i); //内存地址
            i++;
...
void FunHeadAST::generator(){
    tiggerCode.push_back(funName+" ["+convert2string(paraNum)+"]"+ \
    " ["+convert2string(funcName->tNum+funcName->TNum + 8)+"]"); //留8个用于load store a0-a7
    if(paraNum>0){
        for(int i=0;i<paraNum;++i){
            Table tmp("a"+convert2string(i),false);
            funcName->tempVar.insert(pair<string,Table>("p"+convert2string(i),tmp));
        }
    }
}
