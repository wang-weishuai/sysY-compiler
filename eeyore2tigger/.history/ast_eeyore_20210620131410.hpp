...
class Table{
    public:
        ...
        bool isArray;
        int arrNum;
        Table(string name, bool isArray_, int arraynum_):tiggerName(name), \
...
class BaseEeyoreAST{
    public:
        ...
        ASTtype type;
        BaseEeyoreAST* funcName; //指向该函数的FunDefAST
        virtual void generator(){};
        BaseEeyoreAST():type(BaseEeyore){};
};
class ProgramAST:public BaseEeyoreAST{
    public:
        BaseEeyoreAST* globalDecl;
        BaseEeyoreAST* globalInit;
        BaseEeyoreAST* funDefs;
        ProgramAST(BaseEeyoreAST* p,BaseEeyoreAST* d,BaseEeyoreAST* f):globalInit(d),funDefs(f){
        if(p!= nullptr) {
            ProgramAST* pp = (ProgramAST*)p;
            pp->globalDecl= this;
            funcName=pp->funcName;
        }
        else
            funcNam = this;
    }
