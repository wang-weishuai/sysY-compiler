...
void ProgramAST::generator()
{
      for(auto i : globalDecl){
        if(dynamic_cast<DeclAST* >(i)->isArray){
            Table tmp("v"+num2string(vn++), true, dynamic_cast<DeclAST* >(i)->num);
            globalVar.insert(pair<string,Table>(dynamic_cast<DeclAST* >(i)->varName,tmp));
        }else {
            Table tmp("v"+num2string(vn++), false);
            globalVar.insert(pair<string,Table>(dynamic_cast<DeclAST* >(i)->varName,tmp));
        }
    }
