...
# define YYSTACK_BYTES(N) \
     ((N) * (YYSIZEOF (yy_state_t) + YYSIZEOF (TreeNode *)) \
      + YYSTACK_GAP_MAXIMUM)
...
yy_symbol_value_print (FILE *yyo,
                       yysymbol_kind_t yykind, TreeNode * const * const yyvaluep)
{
  FILE *yyoutput = yyo;
  YY_USE (yyoutput);
  if (!yyvaluep)
...
static void
yy_reduce_print (yy_state_t *yyssp, TreeNode * *yyvsp,
                 int yyrule)
{
...
yydestruct (const char *yymsg,
            yysymbol_kind_t yykind, TreeNode * *yyvaluep)
{
  YY_USE (yyvaluep);
  if (!yymsg)
    yymsg = "Deleting";
