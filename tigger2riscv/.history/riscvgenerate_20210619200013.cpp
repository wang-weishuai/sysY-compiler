...
TreeNode * NewNode(Type mode, Type kind)
{
	...
	for(int i = 0; i < 4; ++i)
	{
			...
			t->global = false;
			t->child[i] = NULL;
			if(mode == Stmt_K)t->kind.stmt = kind;
			if(mode == Exp_K) t->kind.exp = kind;
	}
	return t;
}
